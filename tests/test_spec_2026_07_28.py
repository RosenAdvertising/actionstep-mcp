...
def _modern_request(
    ...
    *,
    ...
    request_params["_meta"] = {
        ...
        CLIENT_INFO_META_KEY: {"name": "actionstep-spec-test", "version": "0"},
    }
    headers = {
...
def test_modern_discovery_is_sessionless_and_declares_used_capabilities() -> None:
    ...
    assert result["_meta"][SERVER_INFO_META_KEY]["name"] == "actionstep-mcp"
...
def test_cacheable_results_are_complete_private_and_deterministic() -> None:
    ...
    assert [item["uri"] for item in resources["resources"]] == [
        "actionstep://action_types",
        "actionstep://participant_types",
        "actionstep://security-notes",
    ]
...
def test_resource_read_cache_hints_and_not_found_error(monkeypatch) -> None:
    class StubActionstepClient:
        def list_action_types(self) -> dict[str, Any]:
            return {"actiontypes": [{"id": 1, "name": "Civil"}]}

    monkeypatch.setattr(server, "ActionstepClient", StubActionstepClient)
    found = asyncio.run(
        _post_modern("resources/read", {"uri": "actionstep://action_types"})
    )
    result = _result(found)
    assert result["resultType"] == "complete"
    assert result["ttlMs"] == 0
    assert result["cacheScope"] == "private"
    assert "Civil" in result["contents"][0]["text"]

    missing = asyncio.run(
        _post_modern("resources/read", {"uri": "actionstep://does-not-exist"})
    )
    assert missing.status_code == 400
    assert missing.json()["error"]["code"] == -32602


def test_modern_tool_results_are_structured_and_complete(monkeypatch) -> None:
    class StubActionstepClient:
    ...
    assert '"id": 7' in result["content"][0]["text"]


def test_modern_http_enforces_routing_headers_versions_and_method_errors() -> None:
    named_headers, _ = _modern_request(
        "tools/call", {"name": "list_users", "arguments": {}}
    )
    assert named_headers["mcp-protocol-version"] == PROTOCOL_VERSION
    assert named_headers["mcp-method"] == "tools/call"
    assert named_headers["mcp-name"] == "list_users"
    ...
    named_mismatch = asyncio.run(
        _post_modern(
            "tools/call",
            {"name": "list_users", "arguments": {}},
            header_overrides={"mcp-name": "get_user"},
        )
    )
    assert named_mismatch.status_code == 400
    assert named_mismatch.json()["error"]["code"] == -32020
