#!/usr/bin/env python3
"""Pluggable credential storage for actionstep-mcp.

Secrets (OAuth client ID, client secret, API endpoint) are stored in the
operating system's native secret store via the cross-platform ``keyring``
library:
...
Storage order (read):  OS keyring  ->  process env  ->  ~/.actionstep-mcp/.env file
...
user opts out of keyring with ``ACTIONSTEP_MCP_USE_KEYRING=0``.
...
SERVICE_NAME = "actionstep-mcp"
CONFIG_DIR = Path.home() / ".actionstep-mcp"
ENV_FILE = CONFIG_DIR / ".env"
_USE_KEYRING_FLAG = "ACTIONSTEP_MCP_USE_KEYRING"
# ---------------------------------------------------------------------------
...
def _keyring_enabled() -> bool:
    ...
    Opt out with ``ACTIONSTEP_MCP_USE_KEYRING=0`` (or false/no/off). A failing
    or "null" backend (common on headless Linux) is treated as disabled so we
    fall back to the file store rather than raising.
    """
